...
exports.updateConfig = (app, oldConf, newConf) => {
  ...
  for (var j in newConfigRoutes) {
    addCacheForRoute(app, newConfigRoutes[j].path)
    addCronJob(newConfigRoutes[j].pattern, newConfigRoutes[j].timezone, j)
  }

  for (var k in existingConfigRoutes) {
    addCronJob(existingConfigRoutes[k].pattern, existingConfigRoutes[k].timezone, k)
  }
}
