...
let addRoute = exports.addRoute = (app, path) => {
...
exports.updateConfig = (app, oldConf, newConf) => {
  ...
  for (var j in newRoutes) {
    addRoute(app, newRoutes[j].path)
    addCronJob(newRoutes[j].pattern, newRoutes[j].timezone, j)
  }

  for (var k in existingRoutes) {
    addCronJob(existingRoutes[k].pattern, existingRoutes[k].timezone, k)
  }
}
