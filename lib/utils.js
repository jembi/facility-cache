...
function populateCache (url) {
  ...
  Needle.get(url, function (err, res) {
    if (err) {
      return Logger.error('Fetching facility data failed', { err: err.message })
    }

    if (res.statusCode !== 200) {
      return Logger.error('Non-200 status code: %d', res.statusCode)
    }
    ...
    res.body.rows.forEach(function (row) {
      const key = row[0]
      if (!key) {
        return Logger.error('Facility missing value', { facility: row })
      }
    ...
    subLevelCacheDB.batch(batch, function (err) {
      if (err) {
        return Logger.error('Storing facility data failed', { err: err.message })
      }
