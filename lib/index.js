...
function start (appConfig, callback) {
  ...
  if (apiConf.register) {
    medUtils.registerMediator(apiConf.api, mediatorConfig, (err) => {
      if (err) {
        Logger.error('Failed to register this mediator, check your config', err)
        process.exit(1)
      }
      ...
      medUtils.fetchConfig(openhimConfig, (err, newConfig) => {
        Logger.info('Received initial config:', JSON.stringify(newConfig))
        const config = newConfig
        if (err) {
          Logger.error('Failed to fetch initial config', err)
          process.exit(1)
        } else {
