...
function start (appConfig, callback) {
  ...
  const medConf = mediatorConfig.configure()

  if (apiConf.register) {
    medUtils.registerMediator(apiConf.api, medConf, (err) => {
      if (err) {
        Winston.error('Failed to register this mediator, check your config', err)
        process.exit(1)
      }
      apiConf.api.urn = medConf.urn
    ...
    // default config
    const app = setupApp(medConf.config, appConfig)
...
if (!module.parent) {
  ...
  if (normalConfig && normalConfig.openhim && normalConfig.openhim.register === 'false') {
    normalConfig.openhim.register = false
  }
